...
# _filter_params['output'] = ['enter_mode', 'room_mode', 'socket_connect_cost_time', 'socket_streaminfo_cost_time', 'socket_joinroom_cost_time', 'ui_total_cost_time', 'ui_sub_room_cost_time', 'media_play_cost_time']

# 直播打点
# _filter_params['type'] = ['show', 'visit']
# _filter_params['page'] = ['live_room']
# _filter_params['obj'] = ['watch_live', 'enter_live_room']
# _filter_params['output'] = ['room_enter_time','load_library_time','ui_cost_time','delegate_cost_time','create_viewer_time','all_cost_time','cost_time','stream_type']
...
class logger:
    ...
    def __init__(self):
        ...
        self.file_logger = open(os.path.join(log_dir, "./events-logs-%s.log"%datetime.datetime.now().strftime('%Y%m%d')), 'a+')
        self.detail_file_logger = open(os.path.join(log_dir, "./events-logs-details-%s.log"%datetime.datetime.now().strftime('%Y%m%d')), 'a+')
...
def handle_event(datas, filter=None):
    # Android需要这么处理
    content = str(datas.decode('utf-8').encode('ISO-8859-1'))
    print(type(content))
