...
Email_Clear_EmailBox_Btn_ID = package + FindSource(Source_Email_Clear_EmailBox_Btn_ID)

# Email 登录-清空密码
Source_Email_Clear_PWDBox_Btn_ID = "img_input_delete"
Email_Clear_PWDBox_Btn_ID = package + FindSource(Source_Email_Clear_PWDBox_Btn_ID)
...
Email_Username_Error_ID = package + FindSource(Source_Email_Username_Error_ID)
...
Email_Password_Error_ID = package + FindSource(Source_Email_Password_Error_ID)
...
Email_ShowPassword_Btn_ID = package + FindSource(Source_Email_ShowPassword_Btn_ID)

# Email 确认登录——提交账号密码验证
Source_LogIn_Confirm_Btn_ID = "btw_email_confirm"
LogIn_Confirm_Btn_ID = package + FindSource(Source_LogIn_Confirm_Btn_ID)

# Email 忘记密码
Source_Email_ForgotPassword_Link_ID = "tv_email_forget_password"
Email_ForgotPassword_Link_ID = package + FindSource(Source_Email_ForgotPassword_Link_ID)
...
PhoneHome_Tips_ID = package + FindSource(Source_PhoneHome_Tips_ID)
...
PhoneHome_Describe_ID = package + FindSource(Source_PhoneHome_Describe_ID)

# Phone 当前选择国家国旗（点击可拉起切换弹窗，text="🇨🇳"）
Source_Phone_NowCountryCode_ID = "country_code"
Phone_NowCountryCode_ID = package + FindSource(Source_Phone_NowCountryCode_ID)

# Phone 输入手机号
Source_Phone_input_ID = "com_accountkit_phone_number"
Phone_input_ID = package + FindSource(Source_Phone_input_ID)
# Phone_input_Class = "android.widget.EditText"

# Phone 下一步/继续
Source_Phone_Next_ID = "com_accountkit_next_button"
Phone_Next_ID = package + FindSource(Source_Phone_Next_ID)

# 验证码输入框——首个（1-6对应6个输入框）
Source_Code_FirstInputBox_ID = "com_accountkit_confirmation_code_1"
Code_FirstInputBox_ID = package + FindSource(Source_Code_FirstInputBox_ID)

# 未收到验证码
Source_Not_Received_Code_ID = "com_accountkit_retry_button"
Not_Received_Code_ID = package + FindSource(Source_Not_Received_Code_ID)

# 验证你的手机号{AreaCode}{PhoneNumber}
Source_Verify_Your_Number_ID = "com_accountkit_accountkit_verify_number"
Verify_Your_Number_ID = package + FindSource(Source_Verify_Your_Number_ID)

# 重发短信
Source_Recapture_Code_ID = "com_accountkit_resend_button"
Recapture_Code_ID = package + FindSource(Source_Recapture_Code_ID)
...
QuickLogin_Btn_ID = package + FindSource(Source_QuickLogin_Btn_ID)

# 切换至普通登录
Source_CommonLogin_Btn_ID = "tv_switch_login"
CommonLogin_Btn_ID = package + FindSource(Source_CommonLogin_Btn_ID)

# Development 开发者模式
Source_Development_Btn_ID = "img_star_maker"
Development_Btn_ID = package + FindSource(Source_Development_Btn_ID)

# 快速登录头像
Source_QuickLogin_Image_ID = "img_latest_avatar"
QuickLogin_Image_ID = package + FindSource(Source_QuickLogin_Image_ID)

# 快速登录用户名
Source_QuickLogin_StageName_ID = "tv_latest_name"
QuickLogin_StageName_ID = package + FindSource(Source_QuickLogin_StageName_ID)
