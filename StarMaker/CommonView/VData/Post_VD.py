...
RecordStart_ID = package + FS(Source_RecordStart_ID)
RecordStart_ID_R = [0.503, 0.882, 500]

# 录制页——剩余时间(text=03:36)
Source_RecordTime_ID = "record_time_tv"
RecordTime_ID = package + FS(Source_RecordTime_ID)

# 发布预览页——POST
Source_PostBtn_ID = "btn_post"
PostBtn_ID = package + FS(Source_PostBtn_ID)

# 发布编辑页——SEND
Source_EditSend_ID = "btn_next"
EditSend_ID = package + FS(Source_EditSend_ID)

# 评分页——DONE
Source_ScoreDone_ID = "tv_done"
ScoreDone_ID = package + FS(Source_ScoreDone_ID)
